            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    HStack(spacing: DQ.Spacing.sm) {
                        // Gender ratio badge — visible when balance data is available
                        if balanceEnforcer.isStatsAvailable {
                            HStack(spacing: DQ.Spacing.xxxs) {
                                Image(systemName: "scale.3d")
                                    .font(.system(size: 10, weight: .semibold))
                                Text("\(Int((1.0 - balanceEnforcer.currentRatio) * 100)):\(Int(balanceEnforcer.currentRatio * 100))")
                                    .font(.system(size: 10, weight: .bold, design: .monospaced))
                            }
                            .foregroundStyle(balanceEnforcer.needsFemaleBoost ? DQ.Colors.accentPink : DQ.Colors.textQuaternary)
                            .padding(.horizontal, DQ.Spacing.xs)
                            .padding(.vertical, DQ.Spacing.xxxs)
                            .background(
                                Capsule().fill(
                                    balanceEnforcer.needsFemaleBoost
                                        ? DQ.Colors.accentPink.opacity(0.12)
                                        : DQ.Colors.surfaceElevated
                                )
                            )
                            .accessibilityLabel("Gender ratio: \(Int((1.0 - balanceEnforcer.currentRatio) * 100)) percent female, \(Int(balanceEnforcer.currentRatio * 100)) percent male")
                        }
